...
crit <- rep("min", length(cols) - 1)  # Exclude the geo_id column
...
region_1 <- res_melt %>% 
  filter(county_name %in% c("Lincoln", "Flathead", "Sanders", "Lake", "Minearl", "Missoula", "Ravalli", "Granite", "Deer Lodge", "Powell"))
num_counties <- length(unique(region_1$county_name))
palette_colors <- colorRampPalette(brewer.pal(pmax(num_counties, 3), "Set3"))(num_counties)

# Create the boxplot
ggplot(region_1, aes(x = Closeness, y = geo_id, fill = county_name)) +
  geom_boxplot(outlier.size = 1) +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 9)) +  # Adjusting font size for y-axis labels
  scale_fill_manual(values = palette_colors) +
  labs(y = "Geo ID", x = "Closeness") +
...
vulnerability_colors <- c(
  ...
  "Medium-low" = "#fdd49e", # Light orange
  "Medium-high" = "#fb8726",# Medium orange
  "High" = "#a60000"        # Red
)
predominant_category <- region_1 %>%
  group_by(geo_id) %>%
  summarize(predominant_rank = names(sort(table(rank_grouped), decreasing = TRUE)[1]))
region_1 <- region_1 %>%
  left_join(predominant_category, by = "geo_id")

predominant_category <- region_2 %>%
  group_by(geo_id) %>%
  summarize(predominant_rank = names(sort(table(rank_grouped), decreasing = TRUE)[1]))
region_2 <- region_2 %>%
  left_join(predominant_category, by = "geo_id")

ggplot(region_2, aes(x = Closeness, y = reorder(geo_id, Closeness), fill = predominant_rank)) +
  geom_boxplot(outlier.size = 1) +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 9)) +  # Adjusting font size for y-axis labels
  scale_fill_manual(values = vulnerability_colors) +
  labs(y = "Geo ID", x = "Closeness", fill = "County Name") +
